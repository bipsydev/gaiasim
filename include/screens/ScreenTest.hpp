 ...
 * @brief The testing game screen that renders some stuff and prints some text.
 *
 * Just a testing canvas basically for anything
 * Currently just renders some rectangle grandient primitives and some
 * updating frame info in a text box.
 *
 * @version a_0.0.1-pre
 * @date 2026-05-17
 *
...
class ScreenTest : public Screen
{
   ...
   * each group of 3 vertices as a triangle. So for example, to render a
   * rectangle using 2 triangles, we need 6 vertices (3 for each triangle).
   */
  ...
  static const int GRADIENT_RECT_VERTEX_COUNT
          = GRADIENT_RECT_TRIANGLE_COUNT * 3;
  ...
  std::string   m_text_str;  // This holds the current text string that is
