...
, m_left_sidebar({0, 0.25, 0.25, 0.75},
                 "Left Sidebar",
                 {0xFF, 0x00, 0xCB, 0xFF})
, m_top_sidebar({0, 0, 1.0, 0.25}, "Top Sidebar", {0x00, 0xB4, 0x5D, 0xFF})
, m_window_width{0}
, m_window_height{0}
{}
...
SDL_AppResult ScreenMainGUI::update_layout(Game * game)
{
  ...
  if (not SDL_GetCurrentRenderOutputSize(game->renderer(),
                                         &window_width,
                                         &window_height))
  {
    // Handle error
  ...
  Log::verbose(Log::indent() + 1,
               "Updated layout with window size {}x{}",
               m_window_width,
               m_window_height);

  // regenerate textures based on window size change
  if (changed) return generate_textures(game);
  // continue otherwise
...
SDL_AppResult ScreenMainGUI::event(Game * game, SDL_Event * event)
{
  ...
  // return CONTINUE. In the future, this is where we would handle events like
                                                        ...
                                                        Panel & panel)
{
  ...
  Uint8 brightness = (panel.m_color.r * 0.2126f) + (panel.m_color.g * 0.7152f)
                   + (panel.m_color.b * 0.0722f);
  if (brightness < 128) text_color = {255, 255, 255, 255};  // use white

  SDL_Surface * text_surface = TTF_RenderText_Blended(
          panel_font, panel.m_title.c_str(), 0, text_color
  );
  if (text_surface == nullptr) return Log::error_init("text_surface");

  // Create a texture from the surface
  // This uploads the image data to the GPU for efficient rendering.
  panel.m_title_texture
      ...
      && !panel.m_use_small_font)
  {
    SDL_GetTextureSize(panel.m_title_texture,
                       &panel.m_title_texture_size.w,
                       &panel.m_title_texture_size.h);
  }
                                          ...
                                          GameWorld *    world)
{
  ...
  if (panel.m_title_texture != nullptr)
  {
    // position to render text - we will render the title in the top left corner
    // of the panel with some padding
    ...
    text_rect.x           = std::min(std::max(m_bounds.x + padding_x,
                                              m_bounds.x + (m_bounds.w / 2.0f)
                                                      - (text_rect.w / 2.0f)),
                                     m_bounds.x + m_bounds.w - padding_x);

    text_rect.y = std::min(std::max(m_bounds.y + padding_y, m_bounds.y),
    ...
    if (not SDL_RenderTexture(
                renderer, panel.m_title_texture, &source_rect, &text_rect
        ))
    {
