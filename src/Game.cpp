...
SDL_AppResult Game::new_game(void *& appstate, InitRequest initializations)
{
  ...
  appstate    = game;
  Log::verbose(
          "New Game instance bound to appstate pointer, now initializing Game "
          "with requested initializations..."
  );
  // initialize game state up to the requested init phase, and return result
  SDL_AppResult result = game->init(initializations);

  // log and return
  Log::verbose("Game::new_game factory method completed initializing Game "
               "{} returning {}",
               (result == SDL_APP_CONTINUE) ?
                       "successfully," :
               (result == SDL_APP_SUCCESS) ?
                       "with a \"successful\" termination request? "
                       "weird..." :
               (result == SDL_APP_FAILURE) ? "with failure," :
                                             "with unknown result!",
               result);
...
SDL_AppResult Game::init(InitRequest initializations)
{
  ...
  while (m_inits_complete < initializations)
  {
    ...
    case NONE:
      ...
      {
        Log::error(
                "Error occured while initializing libraries, terminating..."
        );
        return result;
      }
    ...
    case LIBRARIES:
      ...
      {
        Log::error("Error occured while initializing system objects, "
                   "terminating...");
        return result;
      }
    ...
    case SYSTEM_OBJECTS:
      ...
      {
        Log::error(
                "Error occured while initializing game state, terminating..."
        );
        return result;
      }
    ...
    case GAME_STATE:
      ...
      assert(false
             && "Invalid initialization state reached in Game constructor!");
      // GAME_STATE should be the last initialization step, unless we modified
      // it
      static_assert(GAME_STATE == ALL,
                    "Did we add a new initialization step without updating the "
                    "constructor?");
    }
  }
...
SDL_AppResult Game::init_libraries()
{
  ...
  if (not TTF_Init()) return Log::error_init("SDL_ttf");
  Log::verbose(Log::indent() + 1, "SDL_ttf initialized successfully");

  if (not MIX_Init()) return Log::error_init("SDL_mixer");
...
SDL_AppResult Game::init_system_objects()
{
  ...
  // Create a window
  if (not(m_window
          = SDL_CreateWindow("gaiasim engine",
                             1'280,
                             720,
                             // SDL_WINDOW_VULKAN |
  ...
  {
    Log::verbose("TTF_Font \"{}\" loaded at size {} successfully",
                 font_name,
                 24);
  }
  ...
  // TODO dynamic font size based on DPI scale? right now it's just assuming 2.0
  // scale and doubling
...
SDL_AppResult Game::init_game_state()
{
  ...
  Log::verbose("For now, we're going to initialize at a new ScreenTest screen "
               "instance:");
  Screen * screen_test = new ScreenTest(this);
  if (SDL_AppResult result = screen_test->init())
  {
    Log::critical(
            "Error occured while initializing `ScreenTest`, terminating..."
    );
    return result;
  }
  ...
  // TODO use switch_screen() function? encapsulate the window title setting
  // logic
...
SDL_AppResult Game::iterate()
{
  ...
  if (SDL_AppResult result = render()) return result;
  if (SDL_AppResult result = post_render_update()) return result;
...
SDL_AppResult Game::update()
{
  ...
  frame_count.resize(5, ' ');  // Pad frame count to 5 characters for better
                               // readability
