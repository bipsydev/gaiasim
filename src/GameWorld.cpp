...
GameWorld::~GameWorld()
{
  ...
  {
    SDL_DestroyTexture(m_map_texture);
...
SDL_AppResult GameWorld::render(SDL_Renderer * renderer, SDL_FRect * bounds)
{
  ...
  if (m_moved)
  {
    ...
    Log::verbose("Updated player position in m_map_str");
  }
  else
  {
    Log::verbose("Player did not move, m_map_str was not updated");
  }
...
SDL_AppResult GameWorld::test_world_map_operations()
{
  ...
  auto block1 = m_world_map.get_block(ChunkPos{1, 2, 3}, LocalPos{0, 0, 0});
  ...
  auto block8 = m_world_map.set_block(31, 47, 63, BlockID{4});
  ...
  // let's confirm:
  auto block9 = m_world_map.get_block(16, 32, 48);
  Log::debug(Log::indent() + 1, "Got block9 (should be 3): {}", block9);
  auto block10 = m_world_map.get_block(31, 47, 63);
